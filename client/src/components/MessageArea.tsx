                                ...
                                {message.isLoading ? (
                                    ...
                                    message.content ? (
                                        <ReactMarkdown
                                            ...
                                            components={{
                                                h3: ({ node, ...props }) => (
                                                    <h3 className={`text-sm font-semibold ${message.isUser ? 'text-white' : 'text-gray-900'} mt-2 mb-1`} {...props} />
                                                ),
                                                ul: ({ node, ...props }) => (
                                                ...
                                                p: ({ node, ...props }) => (
                                                    <p className="leading-relaxed mb-2 whitespace-pre-wrap" {...props} />
                                                ),
                                                code: ({ node, inline, className, children, ...props }) => (
                                                    <code className={`px-1.5 py-0.5 rounded bg-black/10 ${message.isUser ? 'text-white' : 'text-gray-900'}`} {...props}>
                                                        {children}
                                                    </code>
                                                ),
                                                strong: ({ node, ...props }) => (
                                                    <strong className={`${message.isUser ? 'text-white' : 'text-gray-900'}`} {...props} />
                                                ),
                                                a: ({ node, ...props }) => (
                                                    <a className="underline hover:opacity-80" target="_blank" rel="noreferrer" {...props} />
                                                ),
                                            }}
                                        >
                                            {message.content}
